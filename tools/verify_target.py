...
def compare_identity(o:dict,t:dict)->dict:
 hs={x["algorithm"].lower():x["value"].lower() for x in t["hashes"]};h=t["header"];c={"size":o["size"]==t["size"],"sha1":o["sha1"]==hs.get("sha1"),"sha256":o["sha256"]==hs.get("sha256"),"header_checksum_valid":o["header"]["header_checksum"]==o["header"]["calculated_header_checksum"]}
 for k in ("title","game_code","maker_code","software_version","header_checksum"):c[f"header_{k}"]=o["header"][k]==h[k]
 return c
def main()->int:
